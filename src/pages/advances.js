 ...
 *  Architecture identique à generateReceiptThermal (export.js) :
 *  - Popup window.open('','_blank') sans URL → la lib QR est embarquée inline
 *    (évite tout problème de chemin relatif sur about:blank).
 *  - Renderer SVG forcé → fonctionne dans tous les contextes, 100% offline,
 *    sans canvas, sans CORS, sans toDataURL().
 *  - buildQR() + init() + window.onload calqués sur la réception.
 */
function printAdvanceTicket(advanceId) {
    ...
    // ── Payload QR — format compact identique à la réception ───
